...
namespace TicketSystem.Services
{
    public class TicketService : ITicketService
    {
        ...
        public TicketService(
            ...
            IRepository<Rezervation> rezervationRepository,
            IUnitOfWork uow
        )
        {
        ...
        GetTicketsAsync(SeferViewModel seferViewModel)
        {
            return await _ticketManager.GetTicket(seferViewModel);
        }

        public async Task<LineDataSet>
        GetRouteByLineAsync(LineViewModel lineViewModel)
        {
            return await _ticketManager.GetRouteByLine(lineViewModel);
        }

        public async Task<Otobus>
        GetBusDetailAsync(BusDetailViewModel busDetailViewModel)
        {
            var cacheName =
                $"busDetail_{busDetailViewModel.FirmaNo}_{busDetailViewModel.SeferTakipNo}_{busDetailViewModel.KalkisNoktaID}";
            var bus =
                await _memoryCache
                    .GetOrCreateAsync(cacheName,
                    async entry =>
                    {
                        entry.SlidingExpiration = TimeSpan.FromHours(1);
                        return await _ticketManager
                            .GetBusDetail(busDetailViewModel);
                    });
            return bus;
        }

        public async Task<IslemSonuc>
        BuyTicketAsync(IslemRezervasyonViewModel buyTicketViewModel)
        {
            var resultRezervation =
                await _ticketManager.CreateRezervation(buyTicketViewModel);

            if (!resultRezervation.Sonuc)
                throw new Exception(resultRezervation.Mesaj);
        ...
        public async Task<KaraNoktalar> GetTransferPoint()
        {
            var points =
                await _memoryCache
                    .GetOrCreateAsync("points",
                    async entry =>
                    {
                        entry.SlidingExpiration =
                            TimeSpan
                                .FromHours(_appSettings
                                    .TicketWebServiceCacheTime);
